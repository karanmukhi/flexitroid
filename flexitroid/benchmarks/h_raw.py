...
def algo(data):
    ...
    for obj in data["objectives"]:
        if obj == "cost":
            ...
            x_sol = x.X
            algo_res[f"{obj}_value"] = (
                np.dot(prices[:T_eval], demand_aggr[:T_eval] + x_sol[:T_eval]) * dt
            )
            ...
            x_sol = x.X
            algo_res[f"{obj}_value"] = np.linalg.norm(
                demand_aggr[:T_eval] + x_sol[:T_eval], ord=np.inf
            )
...
def fitHomothetProjectionLinDescisionRule(
    F, H, B, c, dimension, households
):  # calculates optimal scaling factor and offset
    I = np.eye(dimension)
    ...
    aux_IW = model.addMVar(
        shape=(dimension * households, dimension), lb=-gp.GRB.INFINITY
    )
    aux_rV = model.addMVar(shape=dimension * households, lb=-gp.GRB.INFINITY)

    model.setObjective(s, gp.GRB.MINIMIZE)
    model.addConstrs(
        aux_IW[i, j] == I[i, j] for i in range(dimension) for j in range(dimension)
    )
    model.addConstr(aux_rV[0:dimension] == r)
    model.addConstr(aux_rV[dimension:] == -V)
    for i in range(rows_B):
        model.addConstrs(
            G[i, :] @ F[:, j] == B[i, :] @ aux_IW[:, j] for j in range(dimension)
        )
    model.addConstrs(
        aux[i] == gp.quicksum(G[i, k] * H[k] for k in range(4 * dimension))
        for i in range(rows_B)
    )
