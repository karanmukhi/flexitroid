...
class PopulationGenerator:
    ...
    def __init__(self, T: int, pv_count: int = 0, der_count: int = 0, v1g_count: int = 0,
                          e1s_count: int = 0, e2s_count: int = 0, v2g_count: int = 0):
        ...
        self.T = T
        self.poppulation = self.generate_population(pv_count, der_count, v1g_count,
                          e1s_count, e2s_count, v2g_count)
    ...
    def calculate_indiv_sets(self):
        ...
        large =  np.max(arr[np.isfinite(arr)]) * self.T  # Adjust as necessary for your domain
        small = np.min(arr[np.isfinite(arr)]) * self.T   # Adjust as necessary for your domain
    ...
    def generate_population(self, pv_count: int = 0, der_count: int = 0, v1g_count: int = 0,
                          e1s_count: int = 0, e2s_count: int = 0, v2g_count: int = 0) -> Dict[str, List]:
        ...
        if pv_count > 0:
            populations['pv'] = [
                PV(self.T, *sample.pv(self.T))
                for _ in range(pv_count)
            ]

        if der_count > 0:
            populations['der'] = [
                GeneralDER(self.T, *sample.pv(self.T))
                for _ in range(der_count)
            ]
            
        if v1g_count > 0:
            populations['v1g'] = [
                V1G(self.T, *sample.v1g(self.T))
                for _ in range(v1g_count)
            ]
            
        if e1s_count > 0:
            populations['e1s'] = [
                E1S(self.T, *sample.e1s(self.T))
                for _ in range(e1s_count)
            ]
            
        if e2s_count > 0:
            populations['e2s'] = [
                E2S(self.T, *sample.e2s(self.T))
                for _ in range(e2s_count)
            ]
        
        if v2g_count > 0:
            populations['v2g'] = [
                V2G(self.T, *sample.v2g(self.T))
                for _ in range(v2g_count)
            ]
